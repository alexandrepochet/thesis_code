...
class Sentiment(object):
    ...
    def correl(self):
        ...
        if self.dictionary == 'Bing':
            self.correlation['positive'] = np.corrcoef(self.df.Return, self.association['positive'].value)[0,1]
            self.correlation['negative'] = np.corrcoef(self.df.Return, self.association['negative'].value)[0,1]
        else:  
            for sentiment in self.sentiments:
               self.correlation[sentiment] = np.corrcoef(self.df.Return, self.association[sentiment].value)[0,1]       
