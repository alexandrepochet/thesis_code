...
def main(load=1): 
    ...
    file = "C:/Users/alexa/OneDrive/Desktop/Thesis/Bitbucket/thesisforex2/preprocessed_data/data_SVC_ST_10_full_D.txt"
    freq = 'D'
    data_curr = data_currency(file, freq)
    history_index = data_curr.get_date()
    ...
    for train, test in tscv_outer.split(X):
        ...
        best_model, best_accuracy, best_params, acc_per_fold_training_total, acc_per_fold_total = search.fit(X_train, Y_train, 
                                                                                                             previous_training=previous_training,
                                                                                                             previous=previous, neural=True, history_index=history_index)
        print(best_params)
        ...
        X_test_prime = best_model.reshape(X_test_prime, nb_lags_best, val=True)
    ...
    currency.plot_return("lstm_sentiments_daily_NA", dash=True)
    stats = currency.output_summary_stats()
    print(stats)

    currency = currencyReturn(y_test, market_returns, bid_ask, bid_ask_previous)
    currency.run(predicted_directions, t_cost=True)
    currency.plot_return("lstm_sentiments_daily_cost_NA", dash=True)
    stats = currency.output_summary_stats()
    print(stats)
    pdb.set_trace()
    """
    ...
    file = "C:/Users/alexa/OneDrive/Desktop/Thesis/Bitbucket/thesisforex2/preprocessed_data/data_SVC_ST_10_full_H.txt"
    freq = 'H'
    data_curr = data_currency(file, freq)
    history_index = data_curr.get_date()
    ...
    currency = currencyReturn(y_test, market_returns, bid_ask, bid_ask_previous)
    currency.run(predicted_directions, t_cost=False)
    currency.plot_return("lstm_sentiments_hourly_NA", dash=True)
    stats = currency.output_summary_stats()
    print(stats)

    currency = currencyReturn(y_test, market_returns, bid_ask, bid_ask_previous)
    currency.run(predicted_directions, t_cost=True)
    currency.plot_return("lstm_sentiments_hourly_cost_NA", dash=True)
    stats = currency.output_summary_stats()
    print(stats)
    pdb.set_trace() 
    end = time.time()
