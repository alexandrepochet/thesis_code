...
def macd(close, n_fast=12, n_slow=26, fillna=False):
    ...
    """
    emafast = utils.ema(close, n_fast, fillna)
    emaslow = utils.ema(close, n_slow, fillna)
...
def macd_diff(close, n_fast=12, n_slow=26, n_sign=9, fillna=False):
    ...
    emaslow = utils.ema(close, n_slow, fillna)
    macd = emafast - emaslow
    macdsign = utils.ema(macd, n_sign, fillna)
...
def trix(close, n=15, fillna=False):
    ...
    Returns:
        pandas.Series: New feature generated.
    """
    ema1 = utils.ema(close, n, fillna)
    ema2 = utils.ema(ema1, n, fillna)
    ema3 = utils.ema(ema2, n, fillna)
...
def mass_index(high, low, n=9, n2=25, fillna=False):
    ...
    """
    amplitude = high - low
    ema1 = utils.ema(amplitude, n, fillna)
    ema2 = utils.ema(ema1, n, fillna)
    mass = ema1 / ema2
