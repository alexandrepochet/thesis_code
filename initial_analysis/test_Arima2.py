...
def main(): 
     ...
     currency.plot_return("Arima_daily_real", dash=True)
     ...
     currency.plot_return("Arima_daily_cost_real", dash=True)
     ...
     print('Hourly data ')
     print('------------------------------------------------------------------------')
     file = "C:/Users/alexa/OneDrive/Desktop/Thesis/Bitbucket/thesisforex2/preprocessed_data/data_Arima_H.txt"
     freq = 'H'
     data = data_currency(file, freq)
     start = 1
     step = 1
     ...
     currency = currencyReturn(y_test, market_returns, bid_ask, bid_ask_previous)
     currency.run(predicted_directions, t_cost=False)
     currency.plot_return("Arima_hourly_real", dash=True)
     stats, stats_bm = currency.output_summary_stats(bm=True)
     print(stats)
     print(stats_bm)
     ...
     currency = currencyReturn(y_test, market_returns, bid_ask, bid_ask_previous)
     currency.run(predicted_directions, t_cost=True)
     currency.plot_return("Arima_hourly_cost_real", dash=True)
     stats = currency.output_summary_stats()
     print(stats)
     pdb.set_trace()
     end = time.time()
