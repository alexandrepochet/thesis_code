...
def main(): 
    ...
    X = data_curr.get_df()
    X = X.drop(['Direction', 'close_bid_ask', 'Return', 'close_bid_ask_previous'], axis=1)
    X = np.asarray(X)
    ...
    currency.plot_return("SVC_daily_full", dash=True)
    stats = currency.output_summary_stats()
    print(stats)

    currency = currencyReturn(y_test, market_returns, bid_ask, bid_ask_previous)
    currency.run(predicted_directions, t_cost=True)
    currency.plot_return("SVC_daily_cost_full", dash=True)
    stats = currency.output_summary_stats()
    print(stats)
    ...
    currency = currencyReturn(y_test, market_returns, bid_ask, bid_ask_previous)
    currency.run(predicted_directions, t_cost=False)
    currency.plot_return("SVC_hourly_full", dash=True)
    stats = currency.output_summary_stats()
    print(stats)

    currency = currencyReturn(y_test, market_returns, bid_ask, bid_ask_previous)
    currency.run(predicted_directions, t_cost=True)
    currency.plot_return("SVC_hourly_cost_full", dash=True)
    stats = currency.output_summary_stats()
    print(stats)
