...
def main():
    ...
    model = c.Classifier(data_sliced)
    ...
    for classifier in classifiers:
        ...
        print ('---------------\n')
        model.run_classification(50, 50, weighting, n_features, ngram_range, classifier)
        result = model.run("run_classification", nb_processes, training_windows, testing_windows, weighting, 
                            n_features, ngram_range, classifier, type_='initial')
    ...
    plt.xlabel('number of features')
    plt.title('accuracy as a function of the number of features')  
    plt.savefig('./figures/grams_test_idf.jpg', bbox_inches='tight', pad_inches=1)
    plt.close()

    
    # Results on the test set
    data_sliced = data.slice((int(0.8*size)-250), size)   
    model = c.Classifier(data_sliced)
    training_windows =250
    testing_windows = 31
    weighting = 'Count'
    ...
    model.plot_return(title = 'Daily_kneighbors3', dash = False)
    ...
    plt.xlabel('number of features')
    plt.title('accuracy as a function of the number of features')  
    plt.savefig('./figures/grams4.jpg', bbox_inches='tight', pad_inches=1)
    plt.close()
    ...
    plt.xlabel('number of features')
    plt.title('accuracy as a function of the number of features')  
    plt.savefig('./figures/grams4_idf.jpg', bbox_inches='tight', pad_inches=1)
    plt.close()
   
    # Results on the test set
    data_sliced = data.slice((int(0.8*size)-2000), size)   
    model = c.Classifier(data_sliced)
    training_windows =2000
    testing_windows = 2000
    weighting = 'Count'
    ...
    model.plot_return(title = 'Hourly_kneighbors3', dash = True)
