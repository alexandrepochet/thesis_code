...
class GridSearchCV():
    ...
    def fit(self, X_train, Y_train=None, Exog_train=None, X_test=None, Y_test=None, verbose=False, previous_training=None, 
            previous=None, neural=False, SVC_sentiment=False, history_index=None):
        ...
        for params in params_list:
            ...
            for train, test in self.cv_model.split(X_train):
                ...
                if (i==3) or previous is None:
                    ...
                    if neural is False:
                        ...
                        if message is None:
                            ...
                            score_training = history.history['accuracy'] 
                        else:
                            epochs = params_dic["epochs"]
                            score = [0] * epochs
                            score_training = [0] * epochs
                else:
                    if i!=3:
                ...
                if neural is True:
                    acc_per_fold_training.append(score_training)
                fold_no = fold_no + 1
            if neural is True:
                window = 11
                somme_val = np.add(np.add(acc_per_fold[0],acc_per_fold[1]),acc_per_fold[2])/3
                somme_training = np.add(np.add(acc_per_fold_training[0],acc_per_fold_training[1]),acc_per_fold_training[2])/3
                callback_model = callback()
                epoch = callback_model.EarlyStopping(somme_training, somme_val)
                vec_temp = somme_val[0:epoch]
                index = np.argmax(self.moving_average(vec_temp, window))
                smoothed_training = self.moving_average(somme_training, window)
            ...
            if verbose is True:
                ...
                print('------------------------------------------------------------------------')
            if neural is True:
                if np.mean(acc_per_fold) > best_accuracy and smoothed_training[index]>0.55:
        ...
        if self.refit is True:
            if Exog_train is not None:
                ...
                if neural is True:
                    ...
                    self.model.fit(X_train, Y_train, X_test=X_test, Y_test=Y_test, **best_params)
            if neural is True:
                return self.model, best_accuracy, best_params, acc_per_fold_training_total, acc_per_fold_total
            else:
                return self.model, best_accuracy, best_params, acc_per_fold_total
        else:
            return best_accuracy, best_params, acc_per_fold_total
